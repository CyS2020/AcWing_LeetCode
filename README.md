...
3. 整数二分：左段取左，右段取右，左段+1
    - 解释：以上规则是if里第一个判断，左段右段指的是我们想要的那一半(二分的点满足条件那一半)，取左取右指的是边界变化；
    - 边界：目标点是满足条件的那一部分无需额外考虑边界，如果分割点在满足条件的左边或者右边，那么就要进行0/len-1边界的判断
    - 引申：r可以是len - 1也可以是len, 例如中位数中二分时r边界是len
4. 浮点数二分：无边界问题，有精度问题
5. 加减乘除：人工计算，借位进位
...
5. KMP：ne数组记录跳转的位置, 初始值均为-1
   - 思考1：初始化ne数组从1开始, 模式匹配从0开始
   - 思考2：j-1要回到上一个与i-1能匹配的点, 即i-1和ne[j - 1]相等的位置, 继续i与ne[j - 1] + 1开始匹配
...
14. 卡塔兰数：一种操作是另一种操作的前提，两种操作的所有排列组合数
   - 引申：满足条件的01序列，火车进站，不同的二叉搜索树，括号序列
   - 公式：Cn = C2n/n + 1 递推试：c1 = 1; Cn = Cn-1 * (4 * n + 2) / (n + 2)
15. 容斥原理：并集等于奇加偶减
16. 博弈论：SG为0先手必败，异或为0先手必败，SG异或为0先手必败 (SG函数表示集合中最小的自然数)

### 动态规划

0. DP分析法：初始化起始状态，枚举所有的状态组合，再枚举当前的状态转移

- 状态表示：f(i,j) = 集合(ij条件下的所有可能) + 属性(最大值，最小值，数量)
- 状态计算：集合当前i划分子集--热狗划分法，一般原则不重不漏，(max,min可重复, count不能重)
...
3. 区间DP：状态表示为一个区间`i~j`，枚举划分方式k，`i~k`、`k+1~j`; 枚举所有区间 = 区间长度 * 区间左端点
...
14. 一维转二维 x = k / col，y = k % col；二维转一维 k = x * col + y (col表示列数)
...
34. 一般遇到(),[], 左括号和右括号逻辑不一致，直接使用stack保存
35. 左(右)边第一个比他小(大), 这种场景均是单调栈, 左右决定遍历方向, 弹栈直到栈顶比当前值小(大)
### 文字笔记
...
- 拼多多: 最长递增子序列; 页数合并; 非递归实现快排; 数组实现链表的增加删除; 优惠卷扣减
- 微软: 字符串中第一个int值
- 夺畅: 非递归实现前序遍历
- 美团：股票买卖
- 字节：二叉树第k小的节点
- 其他：滑动窗口最大值; 复杂链表复制; 最大岛屿面积; 完全背包
