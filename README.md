...
2. 归并排序：确定中间分界点，递归处理左和右，三个while来归并，左到mid右到r
    - 引申：逆序对的数量
3. 整数二分：左段取左，右段取右，左段+1，等号边界同时取 (>=t表示右段)
4. 浮点数二分：无边界问题，有精度问题
5. 加减乘除：人工计算，借位进位
...
- Dp优化：直接删掉用i层，倒序更新i-1
- 边界条件需要初始化，只考虑两次的状态转移，最新状态由上一次转移过来，最后状态由倒数第二次转移过来
2. 背包模型：01背包、完全背包、多重背包、分组背包
3. 线性DP：状态表示以第i(j)结尾，从i-1(j-1)转移过来
4. 区间DP：状态表示为一个区间i~j，枚举划分方式k，i~k;k+1~j；枚举所有区间=区间长度 * 区间左端点
5. 计数DP：
...
22. 二进制枚举：i < 1 << n; j < n; i >> j & 1 -- 种群;个体;基因; 
23. 二进制优化：k <= n; n -= k; n *= 2;
### 文字笔记
